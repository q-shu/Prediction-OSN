...
def prediction(data_path, plt_name, group_col='Paper_ID'):
    """
    Main workflow: Data loading, group-based Bayesian optimization, training, saving, and plotting.
    ...
    if group_col in df.columns:
        ...
        # Exclude the group column and the target column (last column) from input features X
    ...
    def objective(trial):
        ...
        for train_fold_idx, val_fold_idx in gkf.split(X_train, Y_train, groups=groups_train):
            ...
            model = lgb.LGBMRegressor(**param, random_state=42, verbose=-1)
            model.fit(
                X_t, Y_t,
                eval_set=[(X_v, Y_v)],
                eval_metric='l2',
    ...
    study.optimize(objective, n_trials=20)  # Increase n_trials for more extensive search

    print(f"Best parameters found: {study.best_params}")

    # Retrain the final model on the full training set using best parameters
    best_model = lgb.LGBMRegressor(**study.best_params, random_state=42, verbose=-1)
    best_model.fit(X_train, Y_train)

    # 5. Save the trained model
    model_save_dir = "model/rej"
    if not os.path.exists(model_save_dir):
        os.makedirs(model_save_dir)
    model_file = f"../../{model_save_dir}/gbm.joblib"
    ...
    # 7. Visualization
    plot_result(Y_train, y_pred_train, Y_test, y_pred_test, r2_test, mae_test, rmse_test, plt_name)


def plot_result(y_true_train, y_pred_train, y_true_test, y_pred_test, r2, mae, rmse, save_name):
    """
    Plot actual vs predicted regression scatter plot and save as vector graphic (SVG).
    """
    ...
    ax.text(0.04, 0.94, 'LightGBM', transform=ax.transAxes, fontsize=16, fontweight='bold', va='top')
    ...
    plt.tight_layout()
    fig.savefig(f"{save_dir}/{save_name}.png", dpi=900, format='png')
    plt.show()
...
if __name__ == '__main__':
    ...
    # Ensure 'group_col' matches the column name in your Excel representing literature source/ID
    prediction(
        data_path=DATA_PATH,
        plt_name=PLOT_NAME,
        group_col='Paper_ID'

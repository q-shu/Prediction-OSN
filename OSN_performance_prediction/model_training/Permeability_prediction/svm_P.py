...
def prediction(data_path, plt_name, group_col='Paper_ID'):
    """
    SVR workflow: Group-aware data partitioning, Bayesian optimization with Pipeline, evaluation, and plotting.
    ...
    def objective(trial):
        ...
        # Use Pipeline to prevent data leakage during feature scaling inside CV
        clf = Pipeline([
    ...
    # 5. Save the trained pipeline model
    model_dir = "../../model/per"
    ...
    # 6. Model prediction and evaluation on training and unseen test groups
...
def plot_results(y_true_train, y_pred_train, y_true_test, y_pred_test, r2, mae, rmse, save_name):
    ...
    ax.set_xlabel('Actual Permeance', fontsize=15, labelpad=10)
    ax.set_ylabel('Predicted Permeance', fontsize=15, labelpad=10)

...
def prediction(data_path, plt_name, group_col='Paper_ID'):
    """
    Random Forest prediction workflow: Group-aware data partitioning, Bayesian optimization, training, saving, and plotting.
...
def plot_results(y_true_train, y_pred_train, y_true_test, y_pred_test, r2, mae, rmse, save_name):
    ...
    ax.text(0.04, 0.94, 'RF', transform=ax.transAxes, fontsize=16, fontweight='bold', va='top')
    ...
    plt.tight_layout()
    fig.savefig(f"{save_dir}/{save_name}.png", dpi=900)
    plt.show()
